...
typedef struct Query
{
	...
	/* has modify graph type in whole statement */
	bool		hasGraphwriteClause pg_node_attr(query_jumble_ignore);
	...
	GraphWriteOp g_writeOp pg_node_attr(query_jumble_ignore);
	/* is this for the last clause? */
	bool		g_last pg_node_attr(query_jumble_ignore);
	/* number of clauses that modifies graph before this */
	uint32		g_nr_modify pg_node_attr(query_jumble_ignore);
	/* DETACH DELETE */
	...
	List	   *g_exprs;
	/* expression list for SET/REMOVE */
	List	   *g_sets;
