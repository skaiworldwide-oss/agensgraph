...
transformLabelIdDefinition(CreateStmtContext *cxt, ColumnDef *col)
{
	...
	relname->val.sval.type = T_String;
	relname->val.sval.sval = qname;
	relname->location = -1;
	...
	seqname->val.sval.type = T_String;
	seqname->val.sval.sval = qname;
	seqname->location = -1;
							  ...
							  CreateConstraintStmt *constraintStmt)
{
	...
	cexpr->expr = prop_ref_mutator(constraintStmt->expr, NULL);
 ...
 */
static Node *
prop_ref_mutator(Node *node, void *context)
{
	if (node == NULL)
		return NULL;
 ...
 */
IndexStmt *
transformCreatePropertyIndexStmt(Oid relid, IndexStmt *stmt,
								 const char *queryString)
{
	ParseState *pstate;
	ListCell   *l;
	...
	if (stmt->whereClause)
	{
		stmt->whereClause = prop_ref_mutator(stmt->whereClause, NULL);
...
/*
 * Reduce every label column to a single NOT NULL constraint.
 *
 * Used for the binary-upgrade "CREATE VLABEL/ELABEL ONLY name(labid)" form,
 * where pg_dump re-creates the primary key, defaults and indexes itself as
 * separate commands.  We must not drop the NOT NULL, though: a label column is
 * always NOT NULL (the vertex id via its primary key, everything else
 * explicitly), and since PG18 records NOT NULL as an inheritable pg_constraint
 * row it has to be present on the child before pg_dump restores the parent's
 * key.  pg_dump adds the parent primary key non-recursively (ALTER TABLE ONLY
 * ag_vertex ADD PRIMARY KEY), and ATPrepAddPrimaryKey() refuses to synthesize
 * the child's NOT NULL in that path -- it errors with "column \"id\" of table
 * \"...\" is not marked NOT NULL" instead.  The graph dump path never emits a
 * standalone NOT NULL for label tables, so if we stripped it here it would be
 * lost: the parent key would fail to restore and the child properties/edge
 * endpoints would silently become nullable.  Keeping a NOT NULL (and only a
 * NOT NULL) reproduces the original catalog state once pg_dump re-adds the key
 * and defaults on top.
 */
static void
resetColumnDefsToNotNull(List *columnDefs)
{
	ListCell   *li;

	foreach(li, columnDefs)
	{
		ColumnDef  *col = lfirst(li);
		Constraint *notnull = makeNode(Constraint);

		notnull->contype = CONSTR_NOTNULL;
		notnull->location = -1;

		col->constraints = list_make1(notnull);
	}
}
