					   ...
					   int flags)
{
	...
	if (WALK(query->dijkstraEndId))
		return true;
	if (WALK(query->dijkstraEdgeId))
		return true;
	if (WALK(query->dijkstraLimit))
		return true;
	if (WALK(query->shortestpathEndIdLeft))
		return true;
	if (WALK(query->shortestpathEndIdRight))
		return true;
	if (WALK(query->shortestpathTableOidLeft))
		return true;
	if (WALK(query->shortestpathTableOidRight))
		return true;
	if (WALK(query->shortestpathCtidLeft))
		return true;
	if (WALK(query->shortestpathCtidRight))
		return true;
	if (WALK(query->shortestpathSource))
		return true;
	if (WALK(query->shortestpathTarget))
		return true;
 ...
 */
Node *
raw_expression_tree_mutator_impl(Node *node,
								 tree_mutator_callback mutator,
								 void *context)
{
	/*
...
#define MUTATE(newfield, oldfield, fieldtype)  \
	...
	switch (nodeTag(node))
	{
		...
		case T_InsertStmt:
			{
				...
				MUTATE(newnode->onConflictClause, stmt->onConflictClause,
					   OnConflictClause *);
				MUTATE(newnode->returningClause, stmt->returningClause,
					   ReturningClause *);
		...
		case T_ReturningClause:
			{
				ReturningClause *returning = (ReturningClause *) node;
				ReturningClause *newnode;

				FLATCOPY(newnode, returning, ReturningClause);
				MUTATE(newnode->options, returning->options, List *);
				MUTATE(newnode->exprs, returning->exprs, List *);
				return (Node *) newnode;
			}
			break;
		case T_SelectStmt:
