...
checkValidResultRelation(ResultRelInfo *resultRelInfo, ModifyGraph *plan)
{
	switch (plan->operation)
	{
		case GWROP_CREATE:
			...
			break;
		case GWROP_DELETE:
			CheckValidResultRel(resultRelInfo, CMD_DELETE, ONCONFLICT_NONE, NIL);
			break;
		case GWROP_SET:
			CheckValidResultRel(resultRelInfo, CMD_UPDATE, ONCONFLICT_NONE, NIL);
			break;
		case GWROP_MERGE:
			CheckValidResultRel(resultRelInfo, CMD_INSERT, ONCONFLICT_NONE, NIL);
			if (plan->sets != NIL)
				CheckValidResultRel(resultRelInfo, CMD_UPDATE, ONCONFLICT_NONE,
									NIL);
			break;
		default:
			elog(ERROR, "unknown operation");
