...
getNamespaces(Archive *fout)
{
	...
	appendPQExpBufferStr(query, "LEFT JOIN pg_catalog.ag_graph ag_graph "
...
dumpGraph(Archive *fout, const NamespaceInfo *graphinfo)
{
	...
	if (graphinfo->dobj.dump & DUMP_COMPONENT_ACL)
		dumpACL(fout, graphinfo->dobj.dumpId, InvalidDumpId, "SCHEMA",
				qnspname, NULL, NULL,
				NULL, graphinfo->rolname, &graphinfo->dacl);
...
dumpLabelSchema(Archive *fout, const TableInfo *tblinfo)
{
	...
	if (dopt->binary_upgrade)
		binary_upgrade_set_type_oids_by_rel(fout, q,
											tblinfo);
		...
		(tblinfo->relkind == RELKIND_RELATION))
	{
		...
		if (tblinfo->reloftype)
		{
			...
			appendPQExpBuffer(q, "ALTER TABLE ONLY %s OF %s;\n",
							  qualrelname,
							  getFormattedTypeName(fout, tblinfo->reloftype,
												   zeroIsError));
		}
	}
	...
	for (j = 0; j < tblinfo->numatts; j++)
	{
		...
		if (!shouldPrintColumn(dopt, tblinfo, j) &&
			tblinfo->notnull_constrs[j] != NULL && tblinfo->notnull_islocal[j])
		{
			appendPQExpBuffer(q, "ALTER TABLE ONLY %s ",
							  qualrelname);
