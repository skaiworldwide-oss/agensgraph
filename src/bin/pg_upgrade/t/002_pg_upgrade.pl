...
$newnode->command_ok(
    [
        'psql', '-d', $oldnode->connstr('postgres'),
		'-c',   'SELECT regather_graphmeta()'
    ],
    'Executed regather_graphmeta() function'
);

$newnode->command_ok(
    [
        'psql', '-d', $oldnode->connstr('postgres'),
		'-c',   'SELECT * FROM ag_graphmeta_view',
        '-o',   "$tempdir/meta1.out"
    ],
    'Dumped output of ag_graphmeta_view to meta1.out'
);
...
$newnode->command_ok(
    [
        'psql', '-d', $newnode->connstr('postgres'),
		'-c',   'SELECT regather_graphmeta()'
    ],
    'Executed regather_graphmeta() function'
);
...
$newnode->command_ok(
    ...
    'Dumped output of ag_graphmeta_view to meta2.out'
);
...
# Compare the two dumps, there should be no differences.
my $compare_res_meta = compare("$tempdir/meta1.out", "$tempdir/meta2.out");
is($compare_res_meta, 0, 'old and new dumps match after pg_upgrade');

# Provide more context if the dumps do not match.
if ($compare_res_meta != 0)
{
	my ($stdout, $stderr) =
	  run_command([ 'diff', "$tempdir/meta1.out", "$tempdir/meta2.out" ]);
	print "=== diff of $tempdir/meta1.out and $tempdir/meta2.out\n";
	print "=== stdout ===\n";
	print $stdout;
	print "=== stderr ===\n";
	print $stderr;
