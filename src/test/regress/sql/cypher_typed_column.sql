...
--     written.  A value the property map would accept but the column cannot
--     hold aborts the write.  That is a value of the wrong kind -- a string
--     where the column holds numbers, which is refused by the extraction the
--     column is derived through -- and a value of the right kind the type still
--     cannot take, such as a fractional or out-of-range literal for an int
--     column, which is refused by the cast.
--   * Special floats: NaN / Infinity / -Infinity cannot be jsonb numbers, so a
--     program can only store them as strings -- and a column of numbers refuses
--     a string, so a key holding one of them cannot be promoted at all.
...
-- 14h. A promoted int column constrains its write domain.  It accepts a number
--      the type can hold and rejects everything else the property map would
--      have taken: a string, even one that reads as a number, because a column
--      of numbers holds numbers; and a number the type cannot hold -- a
--      fractional literal (5.0 or 5.5), one out of int range -- or a composite.
--      Either way the whole Cypher write is aborted.  Declaring a typed column
--      constrains what the key may hold.
CREATE (:doc {name:'wok', age:5});          -- accepted
CREATE (:doc {name:'wstr', age:'42'});      -- rejected: a string, not a number
...
-- 14j. A resolved read and a whole-element read cannot disagree about what a
--      property is, because a value of another kind never reaches the column:
--      the write is refused instead.  So the case that would have shown them
--      differing -- the string "42" under an int-declared key -- has no rows to
--      read afterwards.
